...
public class HttpUtil {
    ...
    public static String getJsonData(String jsonParam, String urls) throws Exception {
        ...
        try {
            ...
            if (HttpURLConnection.HTTP_OK == conn.getResponseCode()) {
                ...
                throw new Exception("http通信异常");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        String data02 = StringEscapeUtils.unescapeJava(sb.toString());
        String data03=data02.substring(1);
        String data04=data03.substring(0,data03.length()-2);
        System.out.println("-----"+data04+"======");
        String returnValue;
        try {
            returnValue = JsonPath.read(data04, "$.status");
            String masg=JsonPath.read(data04, "$.msg");
            if ("0x0000".equals(returnValue)){
                return data04;
            }else{
                throw new Exception("调用风控返回失败"+masg);
            }
        }catch (Exception e){
            throw new Exception("调用风控返回异常");
        }
    }
    ...
    public static void main(String[] args) throws Exception{
        ...
        String author = JsonPath.read(data01, "$.outputFields[3].value");
