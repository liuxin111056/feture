...
public class SendTest {
	...
	public static void main(String[] args) {
		...
		for(int i = 1; i <= 1; i++) {
			...
			rabbitTemplate.send("noticeExchange", "nfs.S1002.notice", new Message(requestInfoString.getBytes(), new MessageProperties()));
		}
		...
		for(int i = 1; i <= 1; i++) {
			...
			System.out.println("生产者发送："+ str);
			//已“nfs.meituan.notice” 为routing key，发送到交换上。
			rabbitTemplate.send("noticeExchange", "fs.txncode.notice", new Message(str.getBytes(), new MessageProperties()));
		}
